...
def encode_kcache_group(keys, N, d, H=1):
    ...
    for tok_idx, k_vec in enumerate(keys):
        for i, v in enumerate(k_vec):
            ct[i * N//d + tok_idx] = v
    return ct

def qkt_single_group(cq_rep, kcache_ct, N, d, H, left_token, right_token):
    keys_per_ct = N // d
    result = cq_rep * kcache_ct # elementwise product in N-slot
    result = _sum_by_rot(result, N, d, H)
    print("result:", result)
    mask = _qkt_mask(left_token, right_token, keys_per_ct, H, N)
    print("mask:", mask)
    return result * mask
...
class KCache:
    ...
    def push_back(self, key):
        if self.curr_keys % (self.N // self.d) == 0: # New group needed
            ...
            for i,v in enumerate(key):
                self.ciphertexts[g][i * self.N//self.d + tok_idx] = v
        self.curr_keys += 1
    ...
    def _qkt_single(self, q):
        ...
        for global_ct_indx in range(len(self.ciphertexts)):
            ...
            partial = qkt_single_group(cq_rep, self.ciphertexts[global_ct_indx], self.N, self.d, self.H, first_local_key, first_local_key + num_tok)
            # part = *, *,a3,a4 | *,*,a3,a4
...
def _qkt_mask(left_token, right_token, keys_per_ct, H, N):
    # d = 4, N=8, H=2
    # keys_per_ct = N // d = 2
    # dim_per_head = N // keys_per_ct // H
    ...
    mask = np.zeros(N)
    dim_per_head = N // H
    left_pos_in_head = left_token % dim_per_head
    right_pos_in_head = right_token % dim_per_head
    for i in range(N):
        pos_in_head = i % dim_per_head
        if left_pos_in_head <= pos_in_head < right_pos_in_head:
            mask[i] = 1.0
...
def _gt(Q, K, H, N):
    ...
    for h in range(H):
        ...
        A = Q[:, sl] @ K[:, sl].T
        out[:, h * d_head:h * d_head+A.shape[1]] = A
    return out
...
def main():
    ...
    r0   = acc * _qkt_mask(0, 2, t, H, N)
    print(f"group0   = {r0}")

    kcache_trace_2 = KCache(N, d, Km[2:3], H)
    ck2 = kcache_trace_2.ciphertexts[0]
    r1  = _sum_by_rot(cq_rep * ck2, N, d, H) * _qkt_mask(2, 3, t, H, N)
    r1s = rot(r1, -(1 * t * H))
    print(f"group1→  = {r1s}")
